...
<body style="margin: 20px 0 0 0; padding: 0; box-sizing: border-box;">
        ...
        <table style="margin-left: auto; margin-right: auto;">
            <tr>
                ...
                <td style="padding: 0 15px 0 0;">
                    <center>
                        <font size="4" style="font-family: Times New Roman">Laporan Data Pembelian</font><br>
        ...
        <table style="border-collapse: collapse; margin: 25px 0; font-size: 13px; font-family: sans-serif; margin-left: auto; margin-right: auto;" border="1">
            <thead style="background: #333; color: #fff; text-align: center; font-weight: bold;">
                <tr>
                    ...
                    <th style="padding: 15px 5px;">Kode Faktur</th>
                    <th style="padding: 15px 5px;">Judul</th>
                    <th style="padding: 15px 5px;">Kasir</th>
                    <th style="padding: 15px 5px;">Jumlah Beli</th>
                    <th style="padding: 15px 5px;">Total Harga</th>
                    <th style="padding: 15px 5px;">Bayar</th>
                    <th style="padding: 15px 5px;">Kembalian</th>
                ...
                <tbody style="text-align: center;">
                    ...
                    <tr style="border-bottom: 1px solid #333">
                        ...
                        <td style="padding: 8px 5px;">{{$data->kode_faktur}}</td>
                        <td style="padding: 8px 5px;">{{$data->book['judul']}}</td>
                        <td style="padding: 8px 5px;">{{$data->getKasir['name']}}</td>
                        <td style="padding: 8px 5px;">{{$data->jumlah_beli}}</td>
                        <td style="padding: 8px 5px;">{{$data->total_harga}}</td>
                        <td style="padding: 8px 5px;">{{$data->bayar}}</td>
                        <td style="padding: 8px 5px;">{{$data->kembalian}}</td>
                        <td style="padding: 8px 5px;">{{$data->tanggal}}</td>
                    </tr>
                    @endforeach
                    <tr>
                        <td colspan="2" style="text-align: center; font-weight: bold; padding: 8px 0">Jumlah</td>
                        <th colspan="7" style="text-align: left; padding: 8px 0 8px 20px;">{{$total}}</th>
                    </tr>
                </tbody>
        </table>
    </center>
    <div style="margin-left: 70%">
        <table>
            <tr>
                <td>Di Cetak Oleh : <br><br><br><br>{{ Auth::guard('web')->user()->name }}<br><br>( <i>{{ now()->day }} - {{ now()->month }} - {{ now()->year }}</i> )</td>
            </tr>
        </table>
    </div>
</body>
